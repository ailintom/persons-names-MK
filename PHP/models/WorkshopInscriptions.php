...
class WorkshopInscriptions extends ListModel
{
    ...
    protected $tablename = 'inscriptions INNER JOIN inscriptions_workshops_xref on inscriptions_workshops_xref.inscriptions_id = inscriptions.inscriptions_id';
    public $defaultsort = 'title_sort';

    protected function initFieldNames()
    {
        $this->field_names = new FieldList(['inscriptions_workshops_xref.note', 'status', 'inscriptions.inscriptions_id', 'object_type', 'title', 'material', ' GREATEST(IFNULL(length,0), IFNULL(height,0), IFNULL(width,0), IFNULL(thickness,0))', 'text_content', 'dating', 'inst_prov_temp', 'orig_prod_temp',
            'SELECT CONCAT_WS(" ", IF(CHAR_LENGTH(title_string)>IF(CHAR_LENGTH(personal_name)<14, 31 - CHAR_LENGTH(personal_name), 17), CONCAT("...", RIGHT(title_string, IF(CHAR_LENGTH(personal_name)<14, 31 - CHAR_LENGTH(personal_name), 17))), title_string), personal_name) from attestations WHERE attestations.inscriptions_id  = inscriptions.inscriptions_id LIMIT 1'], ['note', 'status', 'inscriptions_id', 'object_type', 'title', 'material', 'size', 'text_content', 'dating', 'inst_prov_temp', 'orig_prod_temp', 'owner']);
    }
