...
class peopleSameInscr extends peoplePairs
{
    ...
    protected $query1b = 'SELECT attestations_id, attestations_id AS id, inscriptions.inscriptions_id AS inscriptions_id, gender, title_string, title_string_sort, personal_name, personal_name_sort, object_type, title, title_sort, dating, (dating_sort_start+dating_sort_end) as dating_sort, (region_temp) AS region FROM ( attestations INNER JOIN inscriptions ON attestations.inscriptions_id = inscriptions.inscriptions_id)';
    ...
    {
        return '(SELECT SQL_CALC_FOUND_ROWS ' . static::SELLIST . ' FROM (' . $this->query1a . $this->WHERE . ') as att_a INNER JOIN (' . $this->query1b . $this->BWHERE . ') as att_b ON att_a.inscriptions_id = att_b.inscriptions_id AND att_a.attestations_id <> att_b.attestations_id' . static::NOPERSONS . ') UNION '
    ...
    {
        return '(SELECT SQL_CALC_FOUND_ROWS DISTINCT ' . static::SELLIST . ' FROM (' . $this->query2a . $this->WHERE . ') as att_a INNER JOIN (' . $this->query2b . $this->BWHERE . ') as att_b ON att_a.inscr_id = att_b.inscr_id AND att_a.id <> att_b.id)';
    }
}
