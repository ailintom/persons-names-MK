...
/**
 * Description of peopleParent
 *
 * @author Tomich
 */
class peopleParent  extends peoplePairs{
    ...
    protected $query1b = 'SELECT attestations_id, children_temp.parent_id as parent_id, CONCAT( inscriptions.inscriptions_id, "#", attestations_id) AS id, inscriptions.inscriptions_id AS inscriptions_id, gender, title_string, title_string_sort, personal_name, personal_name_sort, object_type, title, title_sort, dating, (dating_sort_start+dating_sort_end) as dating_sort, (COALESCE(inscriptions.origin, inscriptions.production_place, inscriptions.installation_place, inscriptions.provenance)) AS region FROM ( attestations INNER JOIN inscriptions ON attestations.inscriptions_id = inscriptions.inscriptions_id) INNER JOIN children_temp ON attestations.attestations_id = children_temp.child_id';
    ...
    protected $query2b = 'SELECT DISTINCT children_temp.parent_id as parent_id, persons.persons_id as id, 0 as inscriptions_id, gender, title_string, title_string_sort, personal_name, personal_name_sort, "Dossier" as object_type, title, title_sort, dating, (dating_sort_start+dating_sort_end) as dating_sort, region FROM (persons INNER JOIN (SELECT persons_attestations_xref.attestations_id as attestations_id, persons_id, inscriptions_id from persons_attestations_xref INNER JOIN attestations on persons_attestations_xref.attestations_id=attestations.attestations_id WHERE persons_attestations_xref.status="accepted") AS attestations ON persons.persons_id = attestations.persons_id) INNER JOIN children_temp ON persons.persons_id = children_temp.child_id';

    protected function makeSelectFromWhere() {
        return '(SELECT SQL_CALC_FOUND_ROWS ' . static::SELLIST . ' FROM (' . $this->query1a . $this->WHERE . ') as att_a INNER JOIN (' . $this->query1b . $this->BWHERE . ') as att_b ON att_a.attestations_id = att_b.parent_id' . static::NOPERSONS . ') UNION '
    ...
    protected function makeSelectFromWherePersons() {
        return '(SELECT SQL_CALC_FOUND_ROWS ' . static::SELLIST . ' FROM (' . $this->query2a . $this->WHERE . ') as att_a INNER JOIN (' . $this->query2b . $this->BWHERE . ') as att_b ON att_a.id = att_b.parent_id)';
    }
