...
class peopleSpouse  extends peoplePairs{
    ...
    protected $query1b = 'SELECT attestations_id, spouses_temp.second_id as second_id, CONCAT( inscriptions.inscriptions_id, "#", attestations_id) AS id, inscriptions.inscriptions_id AS inscriptions_id, gender, title_string, title_string_sort, personal_name, personal_name_sort, object_type, title, title_sort, dating, (dating_sort_start+dating_sort_end) as dating_sort, (COALESCE(inscriptions.origin, inscriptions.production_place, inscriptions.installation_place, inscriptions.provenance)) AS region FROM ( attestations INNER JOIN inscriptions ON attestations.inscriptions_id = inscriptions.inscriptions_id) INNER JOIN spouses_temp ON attestations.attestations_id = spouses_temp.first_id';
