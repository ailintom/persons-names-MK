...
MK = { // eslint-disable-line no-undef
  init: function () {
    ...
    const initialHeaderBgPos = parseInt(window.getComputedStyle(header).backgroundPositionY, 10)
    ...
    document.querySelectorAll('.filter_content').forEach((el) => {
      el.addEventListener('click', (event) => {
        if (event.target === el) {
          const firstInput = el.querySelector('input:not([type=checkbox]):not([type=radio])')
          if (firstInput) {
            firstInput.focus()
          }
        }
